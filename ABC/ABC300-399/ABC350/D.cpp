...
int main(){
    ...
    ll ans = 0;
    for(int i = 0; i < N; i++){
        // 各連結成分の頂点数-1 <= 辺の数ならば完全グラフにできそう
        int rot = uf.root(i);
        if(roots.count(rot)) continue;
        
        roots.insert(rot);
        ll vertexs = uf.vertexs(rot);
        ll edges = uf.edges(rot);

        ans += vertexs*(vertexs-1)/2 - edges;
        //cout << "vertexs = " << vertexs << " edges = " << edges << endl;
    }
    cout << ans << endl;
}
