...
int main(){
    ...
    int n = init(300001);
    int n2 = init(N);
    SegmentTreeRMQ tree(n2);    // 区間の最小
    SegmentTreeRSQ Num(n);
    vector<int> block(N);
    vector<int> blockNum(300001);
    Num.add(1,N);
    int minus = 0;
    for(int q = 0; q < Q; q++){
        int num; cin >> num;
        if(num == 1){
            ...
            Num.add(bl,-1);
            // cout << "q = " << q << " ";
            // cout << "treeMin = " << tree.getRangeMin(1,N+1) << " bl = " << bl << endl;
            if(tree.getRangeMin(1,N+1) == bl){
                minus++;
            }
        }
        else{
            int y; cin >> y;
            cout << Num.getRangeSum(y+1+minus,n+1) << endl;
        }
    }
    //cout << minus << endl;
}
